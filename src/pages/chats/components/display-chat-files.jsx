...
function DisplayChatFiles({ data, showRemove, isUser }) {
  const { selectedTabId, updateTab } = useChatsProvider();
  const { tab, setFiles } = useChatProvider();
  ...
  const hanldeRemoveFile = () => {
    setFiles((prev) => {
      ...
      updateTab({ files: result }, selectedTabId);
      return result;
    });
  };
...
export default DisplayChatFiles;
