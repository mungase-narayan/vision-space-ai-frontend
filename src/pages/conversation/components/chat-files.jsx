...
function ChatFiles({ data, showRemove }) {
  const [open, setOpen] = useState(false);
  const { setFiles, isConversation } = useConversation();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      {data.type === "image" ? (
        <div
          onClick={() => {
            if (!isConversation) setOpen(true);
          }}
          className={cn(
      ...
      <DialogContent className="sm:max-w-[425px] md:max-w-[800px]">
        ...
        <div className="h-[400px] sm:h-[600px] sm:w-full flex items-center justify-center pt-1">
          {data.type == "file" && !unsupportedTypes.includes(data.extension) ? (
            ...
            <img src={data.blob} alt="image" className="w-full aspect-auto" />
          ) : (
